...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, num_acne_cls=4, pretrained_backbone=True):
    ...
    def train(self, mode=True):
        ...
        return self  # required: eval() returns train(False), must return self
...
def resnet50(num_acne_cls=4, pretrained_backbone=False, **kwargs):
    """Construct a ResNet-50 model.

    Args:
        num_acne_cls: int, number of acne classes
        pretrained_backbone: bool, whether to load ImageNet pretrained backbone
    """
    model = ResNet(Bottleneck, [3, 4, 6, 3], num_acne_cls=num_acne_cls,
